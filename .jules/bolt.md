...
## 2026-05-31 - Replaced array sort with reduce for finding max/min\n**Learning:** Creating a copy of an array and sorting it `[...array].sort((a,b) => ...)[0]` to find a single max or min element has O(N log N) time complexity and O(N) memory allocation overhead, which is inefficient in inner loops.\n**Action:** Use a single-pass `.reduce((max, current) => current.value > max.value ? current : max, array[0])` to find maximum or minimum values in O(N) time without allocating intermediate arrays.
