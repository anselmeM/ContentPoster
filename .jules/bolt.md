...
## 2024-05-17 - O(N) single-pass aggregate metrics calculation
**Learning:** Avoid using multiple `[...array].sort()` passes inside of rendering or state updates to calculate the maximums for multiple properties. This degrades the complexity to O(K * N log N) where K is the number of properties, unnecessarily recreating arrays and thrashing garbage collection.
**Action:** Consolidate these checks into a single O(N) `.reduce()` pass that loops the array once and collects all aggregate maximum values on an accumulator object.
