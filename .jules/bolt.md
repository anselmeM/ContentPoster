...
## 2024-04-18 - [O(4N) chained filters and Redundant Data Extractions in Task Filters]
**Learning:** Found an anti-pattern in `TaskContext.tsx` where `.filter()` was chained multiple times sequentially on the same array to apply active/completed, category, priority, and search text conditions. Additionally, string `.toLowerCase()` conversions for the search query and the task text were executed redundantly *inside* the iteration loop on every check, causing intermediate arrays and significant garbage collection overhead.
**Action:** When filtering an array against multiple active conditions, use a single-pass `filter()` map to process all rules simultaneously (O(N)). Furthermore, pre-compute constants such as sets of lookup values and normalized `.toLowerCase()` strings outside the loop to prevent O(N*M) lookup times and redundant allocations.
