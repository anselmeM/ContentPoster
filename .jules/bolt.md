...
## 2026-04-01 - Optimized Task Filtering O(N*M) lookup to O(N)
**Learning:** In `useTaskFilters` and `taskUtils.filterTasks`, the condition checks for categories and priorities used `.includes()` on arrays for every task in the dataset during a `.filter()` operation. This results in an O(N*M) time complexity where N is the number of tasks and M is the number of selected filters. Additionally, `.toLowerCase()` and `.trim()` operations on the search query were redundantly executed for every task in the loop.
**Action:** Always convert condition arrays into `Set`s outside of array iteration loops to achieve O(1) membership lookups, effectively changing the time complexity to O(N). Additionally, pre-compute and cache derived values (like normalized search queries) outside the loop to avoid redundant operations.
