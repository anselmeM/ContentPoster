...
## 2025-02-28 - Optimize Array Traversals in Content Moderation\n**Learning:** The content moderation service used repeated `Array.indexOf()` calls inside `Array.filter()` causing O(N²) complexity for tracking repeated words. Additionally, it computed `array.filter().length` for early exits which traverses the entire array and allocates new memory.\n**Action:** Replaced O(N²) array lookups with O(N) `Set` and replaced `.filter().length === 0` checks with early-returning `!array.some()` logic.
