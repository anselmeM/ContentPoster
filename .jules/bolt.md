...
## 2026-04-17 - [Remove Redundant Array Allocations in Analytics Chart]
**Learning:** Found an anti-pattern in `PlatformPerformanceChart.jsx` where an array of posts was redundantly grouped into a hash map (`postsByPlatformDate`) using `.reduce()`, and then within a loop over labels, a nested `.reduce()` was used to recalculate engagement sums, despite the presence of an already pre-calculated `engagementMap` just above it. This resulted in O(N) redundant operations and memory overhead from temporary array allocations.
**Action:** Always reuse existing pre-computed hash maps in the current scope for O(1) lookups instead of redundantly re-grouping arrays and performing nested iterations, to eliminate unnecessary memory allocations and time complexity.
