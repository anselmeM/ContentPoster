...
## 2026-05-16 - [Avoid Repeated Array Modifications for Multi-Metric Computation]\n**Learning:** In `PlatformComparison.jsx`, calculating the top platform for 5 distinct metrics was done by invoking `[...array].sort(...)` repeatedly, leading to 5 * O(N log N) time complexity and generating redundant array clones.\n**Action:** Replaced sequential `.sort()` operations with a single-pass `.reduce()` that aggregates all desired metrics concurrently in O(N) time.
