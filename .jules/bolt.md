...
## 2026-04-03 - [O(5N) chained methods in AnalyticsView `analytics` hook]
**Learning:** Found an anti-pattern in `AnalyticsView.jsx` where the `analytics` memoized calculation processed the same `filteredPosts` array 5 separate times using chained array methods (two `.filter()`, two `.reduce()`, and one `.forEach()`), resulting in O(5N) time complexity and redundant garbage collection for intermediate arrays and Date object parsing.
**Action:** When deriving multiple independent statistics from a single dataset, combine the operations into a single O(N) pass using a `for...of` loop or a single `.reduce()`, to prevent repeated iterations and minimize object allocation overhead.
