...
export const notificationService = {
  ...
  markAllAsRead: async (userId) => {
    try {
      ...
      // Reduces latency from O(N) sequential round-trips to concurrent execution, saving ~98% time for 50 items.
      await Promise.all(
        unreadIds.map(id => notificationService.markAsRead(userId, id))
      );
