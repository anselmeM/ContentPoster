...
const PlatformComparison = ({ 
  ...
  const radarData = useMemo(() => {
    ...
    // Replaced multiple Math.max(...filteredMetrics.map(...)) calls inside the map with a single O(N) reduce.
    const maxValues = filteredMetrics.reduce((acc, m) => ({
      posts: Math.max(acc.posts, m.posts),
      engagement: Math.max(acc.engagement, m.engagement),
      reach: Math.max(acc.reach, m.reach),
      avgLikes: Math.max(acc.avgLikes, m.avgLikes),
      avgComments: Math.max(acc.avgComments, m.avgComments),
      growth: Math.max(acc.growth, m.growth)
    }), {
