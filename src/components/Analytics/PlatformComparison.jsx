...
const PlatformComparison = ({ 
  ...
  const radarData = useMemo(() => {
    // Bolt Optimization: Compute max values in a single O(N) pass to avoid O(K*N) complexity
    // and prevent 'Maximum call stack size exceeded' errors from spreading large arrays.
    const maxValues = filteredMetrics.reduce((acc, curr) => {
      acc.posts = Math.max(acc.posts, curr.posts || 0);
      acc.engagement = Math.max(acc.engagement, curr.engagement || 0);
      acc.reach = Math.max(acc.reach, curr.reach || 0);
      acc.avgLikes = Math.max(acc.avgLikes, curr.avgLikes || 0);
      acc.avgComments = Math.max(acc.avgComments, curr.avgComments || 0);
      acc.growth = Math.max(acc.growth, curr.growth || 0);
      return acc;
    }, {
