...
const PlatformComparison = ({ 
  ...
  const radarData = useMemo(() => {
    // Bolt Optimization: Single-pass reduction to compute max values simultaneously
    // Replaced multiple O(N) mapping passes and Math.max calls with a single O(N) reduce
    const maxValues = filteredMetrics.reduce((acc, curr) => ({
      posts: Math.max(acc.posts, curr.posts || 0),
      engagement: Math.max(acc.engagement, curr.engagement || 0),
      reach: Math.max(acc.reach, curr.reach || 0),
      avgLikes: Math.max(acc.avgLikes, curr.avgLikes || 0),
      avgComments: Math.max(acc.avgComments, curr.avgComments || 0),
      growth: Math.max(acc.growth, curr.growth || 0)
    }), {
      posts: 1,
      engagement: 1,
      reach: 1,
